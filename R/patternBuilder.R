...
patternBuilder <- function(viralSubset, classifList, classification, i) {
  ...
  if (classification == "Prophage-like"){
    ...
    else if (startPos != 1) pattern <- c(rep(min(viralSubset[,2]), startPos - 1), seq(maxReadCov, minReadCov, slopingCovSteps))
    else if (endPos != nrow(viralSubset)) pattern <- c(seq(minReadCov, maxReadCov, slopingCovSteps), rep(min(viralSubset[,2]), (nrow(viralSubset) - endPos)))
  } else if (classification == "HighCovNoPattern") pattern <- rep(minReadCov, maxReadCov) ##for NoPattern, min read cov is actually mean read cov and max read cov is the contig length
  return(pattern)
}
