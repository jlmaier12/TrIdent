...
#'@param windowSize The number of basepairs to average read coverage values over. Options are 100, 200, 500, 1000 ONLY.  Default is 1000.
#'@param minBlockSize  The minimum size (in bp) of the Prophage-like block pattern. Default is 10000.
#'@param maxBlockSize The maximum size (in bp) of the Prophage-like block pattern. Default is NA (no maximum).
#'@param minContigLength The minimum contig size (in bp) to perform pattern-matching on. Must be at least 25000. Default is 30000.
...
#'@param suggFiltThresh TRUE or FALSE, Suggest a filtering threshold for TrIdent classifications based on the normalized pattern-match scores. Default is FALSE.
...
#'                    )
TrIdentClassifier <- function(VLPpileup, WCpileup, windowSize=1000, minBlockSize=10000, maxBlockSize=Inf, minContigLength=30000, minSlope=0.001, suggFiltThresh=FALSE, SaveFilesTo){
##error catching
...
##main algorithm start
  ...
  message(paste("Execution time:", round(duration[[1]], 2), units(duration)))
  message(paste(length(which(classificationSummary[[2]][,2] == "Low VLP-fraction read cov")),
               "contigs were filtered out based on low read coverage"))
  message(paste(length(which(classificationSummary[[2]][,2] == "Contig length too small")),
  ...
  if(missing(SaveFilesTo) == FALSE){
    ifelse(!dir.exists(paths=paste0(SaveFilesTo, "\\TrIdentOutput")),
            ...
            stop("'TrIdentOutput' folder exists already in the provided directory"))
    write.table(summaryTable, file=paste0(SaveFilesTo,"\\TrIdentOutput\\TrIdentSummaryTable.csv"), sep=",", row.names=FALSE)
    write.table(summaryTable[which(summaryTable[,2] == "Prophage-like" | summaryTable[,2] == "Sloping" | summaryTable[,2] == "HighCovNoPattern"),],
                file=paste0(SaveFilesTo,"\\TrIdentOutput\\TrIdentSummaryTableCleaned.csv"), sep=",", row.names = FALSE)
    write.table(classificationSummary[[2]], file=paste0(SaveFilesTo,"\\TrIdentOutput\\TrIdentFilteredOutContigs.csv"), sep=",", row.names=FALSE)
