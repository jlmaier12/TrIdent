...
#'                   )
plotTrIdentResults <- function(VLPpileup, WCpileup, TrIdentResults, matchScoreFilter, saveFilesTo) {
  ...
  if (missing(saveFilesTo) == FALSE) {
    ...
    lapply(names(plots),
           function(X)ggsave(filename=paste0(saveFilesTo, "\\TrIdentOutput\\", X, ".png"), plot=plots[[X]],
                             width=8, height=4))
