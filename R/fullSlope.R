...
fullSlope <- function (viralSubset, windowSize, minSlope) {
  ...
  bestMatchInfoLR <- makeFullSlopes("Left", viralSubset, newmax, minReadCov, windowSize)
  bestMatchInfoRL <- makeFullSlopes("Right", viralSubset, newmax, minReadCov, windowSize)
  lapply(seq(newmax, (minReadCov + halfReadCov), -halfToMaxReadCov), function(cov) {
    slopeBottom <- minReadCov
      repeat {
        slopeChangeInfoL <- changeSlope("Left", slopeBottom, halfToMaxReadCov, cov, viralSubset, windowSize)
        slopeChangeInfoR <- changeSlope("Right", slopeBottom, halfToMaxReadCov, cov, viralSubset, windowSize)
        if (abs(slopeChangeInfoL[[7]]) < minSlope | slopeChangeInfoL[[7]] > 0) break
        if (abs(slopeChangeInfoR[[7]]) < minSlope | slopeChangeInfoR[[7]] < 0) break
        if (slopeChangeInfoL[[1]] < bestMatchInfoLR[[1]]) bestMatchInfoLR <<- slopeChangeInfoL
        if (slopeChangeInfoR[[1]] < bestMatchInfoRL[[1]]) bestMatchInfoRL <<- slopeChangeInfoR
