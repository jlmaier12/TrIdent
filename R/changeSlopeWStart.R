...
#' @keywords internal
#'
changeSlopeWStart <- function(leftOrRight, slopeBottom, slopeBottomChange, cov,
                              viralSubset, windowSize){
  ...
  slope <-  ifelse(leftOrRight == "Left",
                   (slopeBottom - cov) / ((nrow(viralSubset) * windowSize) - 5000),
                   (cov - slopeBottom) /  ((nrow(viralSubset) * windowSize) - 5000))
