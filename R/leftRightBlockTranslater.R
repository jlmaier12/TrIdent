...
#' @param minReadCov The baseline value used for the region outside of the block pattern (either 0 or the minimum VLP-fraction read coverage for the contig)
...
leftRightBlockTranslater <- function(viralSubset, pattern, leftOrRight, windowSize, minReadCov, cov, bestMatchInfo, minBlockSize){
  repeat {
    pattern <- if(leftOrRight == "Left") c(pattern[-c(1:(1000 / windowSize))], rep(minReadCov, (1000 / windowSize)))
               else c(rep(minReadCov, (1000 / windowSize)), pattern[-c(((length(pattern)) - ((1000 / windowSize) - 1)):length(pattern))])
    if (length(which(pattern == cov)) < (minBlockSize / windowSize) + 1) break
    diff <-  mean(abs(viralSubset[,2] - pattern))
    startPos <- ifelse(leftOrRight == "Left", 1, (which(pattern == max(pattern))[1]))
    endPos <- ifelse(leftOrRight == "Left", (which(pattern == min(pattern))[1]) - 1, length(pattern))
    if (diff < bestMatchInfo[[1]]) bestMatchInfo <- list(diff, minReadCov, cov, "NA", startPos, endPos, "NA", "Prophage-like")
  }
