...
#' @return List
#' @keywords internal
leftRightBlockTranslater <- function(viralSubset, pattern, leftOrRight,
                                     windowSize, minReadCov, cov, bestMatchInfo,
                                     minBlockSize){
repeat {
    pattern <- if(leftOrRight == "Left") c(pattern[-(seq_len(1000 / windowSize))],
                                           rep(minReadCov, (1000 / windowSize)))
               else c(rep(minReadCov, (1000 / windowSize)),
    ...
    if (diff < bestMatchInfo[[1]]) bestMatchInfo <- list(diff, minReadCov, cov,
                                                         "NA", startPos, endPos,
                                                         "NA", "Prophage-like")
    }
