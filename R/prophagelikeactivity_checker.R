...
prophagelikeactivity_checker <- function(classificationsummary, prophageclassifications, phageread_dataset, microbialread_dataset, windowsize){
  ...
  lapply(1:length(prophageclassifications), function(i) {
    ...
    if (nonmatch_lengthbp < 20000) {
      classificationsummary[which(classificationsummary[,1]==ref_name),7] <<- "CBD" #cant be determined
      classificationsummary[which(classificationsummary[,1]==ref_name),8] <<- "CBD"
    } else {
      ...
      if(ratio > 1.3) {
        ...
        classificationsummary[which(classificationsummary[,1]==ref_name),7] <<- "NO"
      }
    }
  })
