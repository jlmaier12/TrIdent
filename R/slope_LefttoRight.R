...
slope_LefttoRight <- function (viral_subset, windowsize) {
  ...
  bottomtotop_read_cov <- abs(max_read_cov - half_read_cov)/10
  newmax <- max_read_cov+bottomtotop_read_cov
  bottomtotop_read_cov <- abs((newmax-half_read_cov)/10)
  ...
  best_match_info <- list(diff, min_read_cov, newmax, -cov_steps, 1, length(pattern))
  for(cov in seq(newmax, half_read_cov, -bottomtotop_read_cov)) {
    ...
    repeat {
      if (diff < best_match_info[[1]]) {
        best_match_info <- list(diff, slope_bottom, cov, -cov_steps, 1, length(pattern))
      }
  ...
  best_match_results <- append(best_match_info, "Gen/Lat/GTA")
