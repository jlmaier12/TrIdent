...
#'
#' Translates full block-pattern across a contig. Translate the pattern 1000 bp at a time. Stop translating when the pattern is 5000 bp from the end of the contig.
#'
...
blockTranslator <- function(viralSubset, bestMatchInfo, windowSize, pattern){
  ...
  repeat {
    pattern <- c(rep(minPattern, (1000 / windowSize)), pattern[-c((length(pattern) - ((1000 / windowSize) - 1)):length(pattern))])
    ...
    endPos <- which(pattern == max(pattern))[length(which(pattern == max(pattern)))]
    if (diff < bestMatchInfo[[1]]) bestMatchInfo <- list(diff, minPattern, maxPattern, "NA", startPos, endPos, "NA", "Prophage-like")
  }
