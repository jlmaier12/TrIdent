...
fullSlope <- function (viralSubset, windowSize, minSlope) {
  ...
  lapply(seq(newmax, (minReadCov + halfReadCov), -halfToMaxReadCov),
    ...
    slopeBottom <- minReadCov
      repeat {
        slopeChangeInfoL <- changeSlope("Left", slopeBottom, halfToMaxReadCov,
                                        cov, viralSubset, windowSize)
        slopeChangeInfoR <- changeSlope("Right", slopeBottom, halfToMaxReadCov,
                                        cov, viralSubset, windowSize)
