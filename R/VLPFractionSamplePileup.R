...
#'
#' @keywords internal
#' @usage data('VLPFractionSamplePileup')
#' @format ## 'VLPFractionSamplePileup'
...
#' VLP fraction and was generated by purifying VLPs from a conventional mouse fecal homogenate using CsCl density gradient ultracentrifugation.
#' The VLP-fraction extracted DNA was sequenced with Illumina (paired-end mode, 150 bp reads)
#' The sequencing reads were mapped to the associated whole-community assembly using BBMap from the BBTools suite.
#' The bbmap.sh bincov parameter with covbinsize=100 was used to create a pileup file with 100 bp windows. A
...
#' read coverage patterns in the VLP-fraction exemplify TrIdent's pattern-matching functionality across classifications:
...
#' To access the sequencing data used to generate this pileup file and for additional details on the
