...
#'
#' @param leftOrRight
#' Generate pattern for negative slope (left to right, i.e. 'Left') or positive
#' slope (right to left, i.e. 'Right')
#' @param viralSubset
#' A subset of the read coverage pileup that pertains only to the contig
#' currently being assessed
#' @param newMax
#' A value for the top of the sloping pattern that is slightly higher than the
#' maximum coverage value on the viralSubset
...
#' @return List
#' @keywords internal
makeSlopesWStarts <- function(leftOrRight, viralSubset, newMax, minReadCov,
                              windowSize){
  ...
  pattern <- if(leftOrRight == "Left") c(rep(minReadCov, 5000 / windowSize),
                                         seq(newMax, minReadCov, covSteps))
             else c(seq(minReadCov, newMax, covSteps),
  ...
  return(list(diff, minReadCov, newMax, covSteps, startPos,
              endPos, slope, "Sloping"))
}
