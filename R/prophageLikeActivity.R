...
#'
#' Determines whether a detected prophage-like element has read coverage in the whole-community that is either elevated or depressed compared to the average read coverage of the non-prophage region.
#'
#' @param classifSummTable Classification summary table
#' @param prophageLikeClassifList A list containing pattern match information associated with all contigs classified as prophage-like. Generated with the allprophagelike_matches
...
prophageLikeActivity <- function(classifSummTable, prophageLikeClassifList, VLPpileup, WCpileup, windowSize){
  ...
  lapply(seq_along(prophageLikeClassifList), function(i) {
    ...
    if (nonBlockLengthBp < 20000) {
      classifSummTable[which(classifSummTable[,1] == contigName),7] <<- "CBD" #cant be determined
