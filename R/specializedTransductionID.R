...
#' @param noReadCov Number of basepairs of zero read coverage encountered before specialized transduction searching stops. Default is 500.
#' @param specTransLength Number of basepairs of non-zero read coverage needed for specialized transduction to be considered. Default is 2000.
#' @param specificContig Optional, Search a specific contig classified as Prophage-like ("NODE_1").
#' @param matchScoreFilter Optional, Filter plots using the normalized pattern match-scores. A suggested filtering threshold is provided by `TrIdentClassifier()` if `suggFiltThresh=TRUE`.
#' @param logScale TRUE or FALSE, display VLP-fraction read coverage in log10 scale. Default is FALSE.
#' @param SaveFilesTo Provide a path to the directory you wish to save output to. specializedTransductionID() will make a folder within the provided directory to store results.
...
#'                           )
 specializedTransductionID <- function(VLPpileup, TrIdentResults, specificContig, noReadCov=500, specTransLength=2000, matchScoreFilter, logScale=FALSE, SaveFilesTo){
  specTransInfo <- NULL
  ...
  colnames(specTransSumm) <- c("contigName", "specTransduc", "left", "right", "lengthLeft", "lengthRight")
  matchScoreFilter <- ifelse(missing(matchScoreFilter), Inf, matchScoreFilter)
  ...
  lapply(seq_along(TrIdentResultPatterns), function(i) {
    ...
    if(missing(SaveFilesTo) == FALSE){
      ifelse(!dir.exists(paths=paste0(SaveFilesTo, "\\TrIdentSpecTransduction")),
             dir.create(paste0(SaveFilesTo, "\\TrIdentSpecTransduction")),
             stop("'TrIdentSpecTransduction' folder exists already in the provided directory"))
      lapply(names(plots),
             function(X)ggsave(filename=paste0(SaveFilesTo, "\\TrIdentSpecTransduction\\", X, ".png"), plot=plots[[X]],
                               width=8, height=4))
      write.table(specTransSumm, file=paste0(SaveFilesTo,"\\TrIdentSpecTransduction\\SpecTransducSummaryTable.csv"), sep=",", row.names=FALSE)
      return(specTransList)
    } else return(specTransList)
    }
