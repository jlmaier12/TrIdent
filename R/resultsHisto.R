...
#' TRUE or FALSE, Suggest a filtering threshold on the output pattern-match
#' score histogram. Default is FALSE.
...
brks <- seq(min(cleanSummaryTable$normMatchScore),
            max(cleanSummaryTable$normMatchScore), length.out=40)
...
if (suggFiltThresh == TRUE){
    ...
    plot <- ggplot(data=cleanSummaryTable)+
                  ...
                  annotate(geom="label", x=ST, y=max(histogram$counts),
                           label=round(ST, digits=2)) +
                  labs(title="Distribution of Pattern-Match Classifications",
                       x="Normalized Pattern-Match Score",
                       y="count", caption=paste("(Lower scores are better
                                                pattern-matches) \n Suggested
                                                Filtering Threshold=",
                                                round(ST, digits=2), "\n",
    ...
    plot <- ggplot(data=cleanSummaryTable)+
                    ...
                    labs(title="Distribution of Pattern-Match Classifications",
                         ...
                         y="count", caption=paste("(Lower scores are better
                                                  pattern-matches) \n",
                                                  (length(which(summaryTable[,2]
                                                                == "NoPattern"))),
                                                  "contigs were classified as
                                                  'NoPattern' and are not
                                                  displayed on this plot."))+
                    theme(plot.title = element_text(size=16),
                          axis.text = element_text(size=12),
                          axis.title = element_text(size=12),
