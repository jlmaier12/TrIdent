...
#'
#' Calculate the VLP-fraction:whole-community read coverage ratio for every contig using the median
#' read coverage values. If the ratio is greater than 2 (i.e VLP-fraction read coverage
#' is, on average, at least double the whole-community read coverage), then the contig is classified as HighCovNoPattern
#'
...
VLPtoWCRatioCalc <- function(classifSumm, WCpileup, VLPpileup){
  ...
  if(length(noneClassifIdxs) == 0) return(classifSumm)
  lapply(seq_along(noneClassifIdxs), function(p) {
    ...
    VLPtoWCratio <- round(median(viralSubset[,2] / median(microbialSubset[,2])), digits = 4)
    classifSumm[i,2] <<- ifelse(VLPtoWCratio > 2, "HighCovNoPattern", "NoPattern")
