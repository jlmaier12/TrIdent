...
#' @param zoom
#' The number of rows outside the start and stop positions of the block pattern
#' to zoom-in on
#' @param windowSize The window size used to re-average read coverage pileups
#' @return Dataframe
#' @keywords internal
prophageLikeZoom <- function (viralSubset, classificationPatterns, i, zoom,
                              windowSize) {
startRowIdx <- classificationPatterns[[i]][[5]] * windowSize / 100
endRowIdx <- classificationPatterns[[i]][[6]] * windowSize / 100
zoomStartRowIdx <- ifelse((startRowIdx - zoom) < 1, 1, (startRowIdx - zoom))
zoomEndRowIdx <- ifelse((endRowIdx + zoom > nrow(viralSubset)),
                          nrow(viralSubset), (endRowIdx + zoom))
