...
#' @keywords internal
fullSlope <- function (viralSubset) {
  ...
  bestMatchInfoLR <- makeFullSlopes("Left", viralSubset, newmax, minReadCov)
  bestMatchInfoRL <- makeFullSlopes("Right", viralSubset, newmax, minReadCov)
  lapply(seq(newmax, (minReadCov + halfReadCov), -halfToMaxReadCov), function(cov) {
    slopeBottom <- minReadCov
      repeat {
        slopeChangeInfoL <- changeSlope("Left", slopeBottom, halfToMaxReadCov, cov, viralSubset)
        slopeChangeInfoR <- changeSlope("Right", slopeBottom, halfToMaxReadCov, cov, viralSubset)
        if (abs(slopeChangeInfoL[[7]]) < 15 / 100000 | slopeChangeInfoL[[7]] > 0) break
        if (abs(slopeChangeInfoR[[7]]) < 15 / 100000 | slopeChangeInfoR[[7]] < 0) break
