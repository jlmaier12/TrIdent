...
slope_LefttoRight <- function (viral_subset, windowsize) {
  ...
  pattern <- seq(newmax,min_read_cov, -cov_steps) #different in each function
  ...
  lapply(seq(newmax, (min_read_cov+half_read_cov), -bottomtotop_read_cov), function(cov) {
    ...
    pattern <- seq(cov,slope_bottom, -cov_steps) #different in each function
    slope <- (slope_bottom-cov)/(nrow(viral_subset)-1) #different in each function
    if (abs(slope) < 15/100000 | slope > 0) next #different in each function
    repeat {
