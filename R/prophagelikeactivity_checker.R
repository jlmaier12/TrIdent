...
prophagelikeactivity_checker <- function(classificationsummary, prophageclassifications, phageread_dataset, microbialread_dataset, windowsize){
  ...
  for (i in seq(1,length(prophageclassifications),1)) {
    ...
    ref_name <- prophageclassifications[[i]][[8]]
    ...
    if (nonmatch_lengthbp < 20000) {
      classificationsummary[which(classificationsummary[,1]==ref_name),4] <- "CBD" #cant be determined
    } else {
      microbial_subset <- microbialread_dataset[which(microbialread_dataset[,1] == prophageclassifications[[i]][[8]]),]
      ...
      ratio <- mean(prophage_region)/mean(nonprophage_region)
      if(ratio > 1.3) {
        classificationsummary[which(classificationsummary[,1]==ref_name),4] <- "YES"
      } else {
        classificationsummary[which(classificationsummary[,1]==ref_name),4] <- "NO"
      }
    }
  }
  return(classificationsummary)
