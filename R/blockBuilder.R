...
blockLengthFull <- ifelse((nrow(viralSubset) - (10000 / windowSize)) >
                            (maxBlockSize / windowSize),
                          maxBlockSize / windowSize,
                          nrow(viralSubset) - (10000 / windowSize))
nonBlockFull <- nrow(viralSubset) - (blockLengthFull + (5000 / windowSize))
bestMatchInfoFull <- makeBlockPattern(viralSubset, windowSize, "Full",
                                      blockLengthFull, nonBlockFull,
                                      minReadCov, startingCovs[1])[[1]]
bestMatchInfoRight <- makeBlockPattern(viralSubset, windowSize, "Right",
                                       blockLength, nonBlock, minReadCov,
                                       startingCovs[1])[[1]]
bestMatchInfoLeft <- makeBlockPattern(viralSubset, windowSize, "Left",
                                      blockLength, nonBlock, minReadCov,
                                      startingCovs[1])[[1]]

lapply(seq_along(startingCovs), function(i) {
    cov <- startingCovs[[i]]
    patternFull <- makeBlockPattern(viralSubset, windowSize, "Full",
                                    blockLengthFull, nonBlockFull, minReadCov,
                                    cov)[[2]]
    patternRight <- makeBlockPattern(viralSubset, windowSize, "Right",
                                     blockLength, nonBlock,
                                     minReadCov, cov)[[2]]
    patternLeft <- makeBlockPattern(viralSubset, windowSize, "Left",
                                    blockLength, nonBlock,
                                    minReadCov, cov)[[2]]
    bestMatchInfoLeft <<- leftRightBlockTranslater(viralSubset, patternLeft,
                                                   "Left", windowSize,
                                                   minReadCov, cov,
                                                   bestMatchInfoLeft,
                                                   minBlockSize)
    bestMatchInfoRight <<- leftRightBlockTranslater(viralSubset, patternRight,
                                                    "Right", windowSize,
                                                    minReadCov, cov,
                                                    bestMatchInfoRight,
                                                    minBlockSize)
    repeat {
        bestMatchInfoFull <<- blockTranslator(viralSubset, bestMatchInfoFull,
                                             windowSize, patternFull)
        middleRows <- which(patternFull == cov)
        if (length(middleRows) < (minBlockSize / windowSize) + 1) break
        patternFull <- c(patternFull[-c(middleRows[2]:
