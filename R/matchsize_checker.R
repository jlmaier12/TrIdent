...
matchsize_checker <- function(classificationsummary, transductionclassification_list, windowsize){
  ...
  lapply(1:length(transductionclassification_list), function(i) {
    ...
    classificationsummary[which(classificationsummary[,1]==ref_name),4] <<- (length(c(start_pos:end_pos))-1) *windowsize
    classificationsummary[which(classificationsummary[,1]==ref_name),5] <<- start_pos *windowsize
    classificationsummary[which(classificationsummary[,1]==ref_name),6] <<- end_pos *windowsize
  })
