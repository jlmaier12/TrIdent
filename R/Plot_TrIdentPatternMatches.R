...
#' @param matchscorefilter Filter the results using a pattern match threshold. The pattern match threshold can be defined using the density plot output with TrIdent_Classifier()
#' @param SaveFilesTo Provide a path to the directory you wish to save TrIdent plots to. TrIdent will create a folder within the provided directory to store results. This parameter is useful if running TrIdent on the command line.
#' @export
#'
#' @examples
#' \dontrun{
...
Plot_TrIdentPatternMatches <- function(VLP_pileup, WC_pileup, transductionclassifications, matchscorefilter, SaveFilesTo, cleanup=TRUE) {
  ...
  if (FilePath != 0) {
    ifelse(!dir.exists(paths=paste0(FilePath, "\\TrIdentOutput")), dir.create(paste0(FilePath, "\\TrIdentOutput")), print("'TrIdentOutput' folder exists already in the provided directory"))
    lapply(names(plots),
