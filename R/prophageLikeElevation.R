...
#' @return dataframe
#' @keywords internal
prophageLikeElevation <- function(classifSummTable, prophageLikeClassifList,
                                  VLPpileup, WCpileup, windowSize){
message("Identifying highly active/abundant or heterogenously integrated
        Prophage-like elements")
classifSummTable$proLikeWCReadCov <- rep(NA, nrow(classifSummTable))
classifSummTable$proLikeWCReadCovRatio <- rep(NA, nrow(classifSummTable))
...
lapply(seq_along(prophageLikeClassifList), function(i) {
    ...
    if(ratio > 1.15) classifSummTable[which(classifSummTable[,1] == contigName),8] <<- "Elevated"
    else if (ratio < 0.75) classifSummTable[which(classifSummTable[,1] == contigName),8] <<- "Depressed"
    else classifSummTable[which(classifSummTable[,1] == contigName),8] <<- "None"
})
