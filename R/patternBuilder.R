...
if (classification == "Prophage-like"){
    if (startPos == 1) pattern <- c(rep(maxReadCov, endPos),
                                    rep(minReadCov,
                                        (nrow(viralSubset) - endPos)))
    else if (endPos == nrow(viralSubset)) pattern <- c(rep(minReadCov,
                                                           startPos),
                                                       rep(maxReadCov,
                                                           (nrow(viralSubset) - startPos)))
    else {
      blockRegion <- endPos - startPos
      pattern <- c(rep(minReadCov, startPos), rep(maxReadCov, blockRegion),
                   rep(minReadCov,
    ...
    else if (startPos == 1 & endPos == nrow(viralSubset) & slopingCovSteps > 0) pattern <- seq(minReadCov, maxReadCov, slopingCovSteps)
    else if (startPos != 1) pattern <- c(rep(min(viralSubset[,2]),
                                             startPos - 1),
                                         seq(maxReadCov,
                                             minReadCov,
                                             slopingCovSteps))
    else if (endPos != nrow(viralSubset)) pattern <- c(seq(minReadCov,
                                                           maxReadCov,
                                                           slopingCovSteps),
                                                       rep(min(viralSubset[,2]),
                                                           (nrow(viralSubset) - endPos)))
    ##for NoPattern, min read cov=median read cov and max read cov=contig length
  } else if (classification == "HighCovNoPattern") pattern <- rep(minReadCov,
                                                                  maxReadCov)
return(pattern)
}
