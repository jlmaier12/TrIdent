...
#'                   )
plotTrIdentResults <- function(VLPpileup, WCpileup, TrIdentResults,
                               matchScoreFilter, saveFilesTo) {
...
plots <- lapply(seq_along(cleanSummaryTable), function(i){
    ...
    if (classification == "Sloping") subtitleInfo <- paste("Slope:",
                                                           patternMatchInfo[10])
    ...
    wholecomm_plot <- ggplot(data=microbialSubset, aes(x=position, y=coverage))+
      ...
      labs(title=paste(contigName, "Classification:", classification),
           subtitle=paste("Matching-region size (bp):", matchLength,
                          subtitleInfo),
           x=" ", y="Whole-community \n read coverage") +
