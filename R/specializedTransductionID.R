...
#' @param noReadCov
#' Number of basepairs of zero read coverage encountered before specialized
...
#' transduction to be considered. Default is 2000.
#' @param specificContig
#' Optional, Search a specific contig classified as Prophage-like ("NODE_1").
#' @param matchScoreFilter
#' Optional, Filter plots using the normalized pattern match-scores. A
#' suggested filtering threshold is provided by `TrIdentClassifier()` if
#' `suggFiltThresh=TRUE`.
#' @param logScale
#' TRUE or FALSE, display VLP-fraction read coverage in log10 scale. Default is
#' FALSE.
#' @param SaveFilesTo
#' Provide a path to the directory you wish to save output to.
#' specializedTransductionID() will make a folder within the provided directory
#' to store results.
...
#'                           )
 specializedTransductionID <- function(VLPpileup, TrIdentResults,
                                       specificContig, noReadCov=500,
                                       specTransLength=2000, matchScoreFilter,
                                       logScale=FALSE, SaveFilesTo){
  specTransInfo <- NULL
  ...
  lapply(seq_along(TrIdentResultPatterns), function(i) {
      ...
      if(is.na(specificContig)) {
        if (classification == "Prophage-like" & normMatchScore < matchScoreFilter) {
          specTransInfo <<- specTransductionSearchAndPlot(contigName, VLPpileup,
          ...
          J <<- J+1
              ...
              specTransInfo <<- specTransductionSearchAndPlot(contigName,
                                                              VLPpileup,
                                                              TrIdentResultPatterns,
                                                              TrIdentResultSumm,
                                                              windowSize, i,
    ...
    if (specTransCount == 0) stop ("Selected contig is either not prophage-like,
                                   spelled incorrectly or has a match score
                                   below the chosen matchScoreFilter")
    message(specTransCount, " contigs have potential specialized transduction")
    ...
    if(missing(SaveFilesTo) == FALSE){
      ifelse(!dir.exists(paths=paste0(SaveFilesTo, "\\TrIdentSpecTransduction")),
             ...
             stop("'TrIdentSpecTransduction' folder exists already in the
                  provided directory"))
      lapply(names(plots),
             function(X)ggsave(filename=paste0(SaveFilesTo,
                                               "\\TrIdentSpecTransduction\\", X,
                                               ".png"),
                               plot=plots[[X]], width=8, height=4))
      write.table(specTransSumm, file=paste0(SaveFilesTo,
