...
#' @param classifPatternMatches The pattern match information associated with each contig classified as prophage-like,  sloping, or HighCovNoPattern
...
#' @param noReadCov How many bp of no read coverage are encountered before searching stops? Default is 500.
#' @param specTransLength  How many bp of read coverage to look for outside of prophage borders? Default is 2000.
...
specTransductionSearchAndPlot <- function(contigName, VLPpileup, classifPatternMatches, classifSumm, windowSize, i, noReadCov, specTransLength, logScale){
  ...
  viralSubsetZoom <- prophageLikeZoom(VLPpileup[which(VLPpileup[,1] == contigName),], classifPatternMatches, i, 500, windowSize)
  borders <- prophageLikeBorders(VLPpileup[which(VLPpileup[,1] == contigName),], classifPatternMatches, i, windowSize)
  ...
  specTransSumm[2] <- ifelse((startSearch[[2]] - startSearch[[1]] >= specTransLength / 100 | endSearch[[2]] - endSearch[[1]] >= specTransLength / 100), "yes", "no")
  ...
  plot <- (ggplot(data=as.data.frame(viralSubsetZoom), aes(x=position, y=coverageType))+
             ...
             geom_vline(xintercept=SpecTransLeft, color="red", alpha=alphaL, linewidth=1)+
             ...
             labs(title=paste(contigName, classifPatternMatches[[i]][[8]], prophageLikeInfo),
                  ...
                  y=paste("VLP-Fraction Read Coverage", ifelse(logScale == TRUE, "(Log10)", ""))))
